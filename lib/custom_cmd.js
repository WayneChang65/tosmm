...
let gDoc_cc = {};
let gAuth_cc = {};	// 每個 GoogleSpreadsheet 對應的 GoogleAuth
...
async function _initBasicData(_groupConst) {
	...
	gAuth_cc[sIdx] = new JWT({
		email: g_secret.client_email,
		key: g_secret.private_key,
		scopes: ['https://www.googleapis.com/auth/spreadsheets'],
	});
	gDoc_cc[sIdx] = new GoogleSpreadsheet(spreadsheetId, gAuth_cc[sIdx]);
	gCustomCMD[sIdx] = [];
	gCustomResult[sIdx] = [];
...
async function _getCustomCmd_FromGoogleSpreadSheet(_idx) {
	...
	try {
		...
		const sheet = gDoc_cc[_idx].sheetsByIndex[worksheetID];
