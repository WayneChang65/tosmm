...
async function _pttCrawler() {
	...
	ptt.urls = await shortenURL_forArray(ptt.urls, 1);
	return ptt;
}
...
async function _bahaCrawler() {
	...
	baha.urls = await shortenURL_forArray(baha.urls, 2);
	return baha;
}
...
async function _tosCrawler() {
	...
	tos.urls = await shortenURL_forArray(tos.urls, 3);
	return tos;
}
...
async function _g8gCrawler() {
	...
	g8g.urls = await shortenURL_forArray(g8g.urls, 4);
	return g8g;
}
