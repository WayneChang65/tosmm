...
const idb_crawler2 = require('./crawler_idb2.js');
const machinery_crawler = require('./crawler_machinery.js');
const sm_crawler2 = require('./crawler_sm2.js');
...
async function _idbCrawler() {
	await idb_crawler.initialize();
	let idb = await idb_crawler.getResults();
	await idb_crawler.close();
	idb.urls = await shortenURL_forArray(idb.urls);
	return idb;
}

async function _idbCrawler2() {
	let idb = await idb_crawler2.getResults();
	/*
...
async function _machineryCrawler() {
	try {
		await machinery_crawler.initialize();
		let machinery = await machinery_crawler.getResults();
		await machinery_crawler.close();
		machinery.urls = await shortenURL_forArray(machinery.urls);
		return machinery;
	} catch(e) {
		console.log(e);
		return undefined;
...
async function _smCrawler2() {
	let sm = await sm_crawler2.getResults();
	return sm;
}
...
module.exports = {
	...
	idbCrawler2 : _idbCrawler2,
	machineryCrawler : _machineryCrawler,
	smCrawler2 : _smCrawler2,
