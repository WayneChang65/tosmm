...
let groupDB_path = '../json/tos_group_db.json';
let groupDB_save_path = './json/tos_group_db.json';
let groupDB;

let usrDB_path = '../json/tos_ana_db.json';
let usrDB_save_path = './json/tos_ana_db.json';
const tos_ana_db = require(usrDB_path);
let usrDB = tos_ana_db;
	...
	return new Promise((resolve, reject) => {
		...
		//fs.writeFile(null, JSON.stringify(_db), function(err){
			if(err) {
				...
				if (dbName[dbName.length - 1] === '/json/tos_group_db'){
					groupDB = _db;
				}else if(dbName[dbName.length - 1] === '/json/tos_ana_db'){
					usrDB = _db;
				}
...
function _init(_groupDB_path = '../json/tos_group_db.json',
	_groupDB_save_path = './json/tos_group_db.json') {
