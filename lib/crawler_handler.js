...
const flex_crawler = require('./flex_crawler2.js');
...
async function _updateCrawlerInfo(_event, _bot) {
		...
		try {
			...
			if (aryNewMsgs.length !== 0) {
				...
				aryNewMsgs.forEach(async (item) => {
					...
					const aryTosmmSkipGids = tosmm_skip.getSkipTosmmGids();
					...
					for (let i = 0; i < aryTosmmSkipGids.length; i++) {
						_bot.push(aryTosmmSkipGids[i],
							flex_crawler.msg(item.title, decodeURIComponent(shortHref), 'IDB'));
					}
	...
	*/
	...
	// * ==== 產創平台 TIIP Crawler ====
	crawlerObj.tiip =  await crawler.tiipCrawler();
	fmlog('sys_msg', [s.ch_tiip, s.ch_init]);
	setInterval(async () => {
		try {
			const newTiip = await crawler.tiipCrawler();
			let aryNewMsgs = basic_f.checkNewItemFromArray(crawlerObj.tiip, newTiip);
			if (aryNewMsgs.length !== 0) {
				crawlerObj.tiip = newTiip;
				aryNewMsgs.forEach(async (item) => {
					const shortHref = await crawler.shortenURL(item.href);
					const aryTosmmSkipGids = tosmm_skip.getSkipTosmmGids();
					// _bot.push('Cedb278e86630179f19a395f9f7984a62',
					// 	flex_crawler.msg(item.title, decodeURIComponent(shortHref), 'TIIP'));
					...
					const dispTitle = (item.title.length > 10) ? item.title.slice(0, 9) : item.title;
					fmlog('crawler_msg', [s.ch_refresh, s.ch_tiip,
						dispTitle + '...', shortHref, basic_f.getCurrentDateTime()]);
				});
			} else {
				fmlog('crawler_msg', [s.ch_nocmd, s.ch_tiip,
					'-'.repeat(26), ' ', basic_f.getCurrentDateTime()]);	
			}
		} catch (err) {
			fmlog('error_msg', [s.ch_tiip, s.ch_crawlererror, ' ']);
		}
	}, 1000 * 60 * 30 / debug_shortenDurationMultiplier);	// 30分鐘	
}
