...
const _anAreaItem = {
	...
	"contents": [{
		...
		"contents": [{
			"type": "icon",
			"url": c.TOSMM_TORO_PIC_PATH_FLEX + "weather/sunny.png",
			"size": "xxl"
		},
		{
			"type": "text",
...
function _getWeatherSignUrl(_rainPossibility) {
	...
	if (_rainPossibility < 30) {
		weatherSignUrl = c.TOSMM_TORO_PIC_PATH_FLEX + 'weather/sunny.png';
	} else if (_rainPossibility >= 30 && _rainPossibility < 70){
		weatherSignUrl = c.TOSMM_TORO_PIC_PATH_FLEX + 'weather/cloudy.png';
	} else {
		weatherSignUrl = c.TOSMM_TORO_PIC_PATH_FLEX + 'weather/rainy.png';
	}
...
function _msg(_alt_txt, _date, _area_items) {
	...
	for (let i = 0; i < _area_items.length; i++) {
		...
		anAreaItem.contents[1].contents[1].contents[0].text = '濕度 ' + _area_items[i].rain;
