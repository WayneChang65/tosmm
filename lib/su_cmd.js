...
const tos_db = require('../json/tos_db.json');
...
function _do(_event, _bot) {
	...
	case 'w小妹計':
		total_ToroCards = parseInt(tos_db.total_torocards); // 統計所有抽的卡數(加倍 + 未加倍)
		toroCards_t = parseInt(tos_db.torocards_t); 		// 統計所有抽的卡數(未加倍)
		toroCards_tt = parseInt(tos_db.torocards_tt); 		// 統計所有抽的卡數(加倍)
		total_TfCards = parseInt(tos_db.total_tfcards);		// 統計所有討伐開房圖數
		total_KuziCards = parseInt(tos_db.total_kuzicards);	// 統計所有60甲子籤數
		total_AsaCards = parseInt(tos_db.total_asacards);	// 統計所有淺草觀音100的籤數
		total_tg = parseInt(tos_db.total_tg);				// 統計所有抽美女的數量
		total_tb = parseInt(tos_db.total_tb);				// 統計所有抽帥哥的數量
		toroCards_tsm = parseInt(tos_db.total_tsm);			// 統計所有抽的卡數(超人卡匣)
		msgR = '小妹起床時間：\n' + m_DateTime + '\n\n'
			+ '未加倍抽卡數 = ' + toroCards_t + '\n'
			+ '加倍抽卡數 = ' + toroCards_tt + '\n'
			+ '總數 = ' + total_ToroCards + '\n\n'
