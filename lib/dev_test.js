...
function _do(_event, _bot) {
	...
	switch (msg) {
	case 'me':
		flex_level.msg_test(_event, _bot);			// 測試
		break;	
	case 'l!savedb':
		flex_level.saveUDB();						// 測試
		break;		
	case '?test0':
		usr_mgr.modify_userDB();
		break;
	case 'l!001':
		flex_level.getUserLevel(_event);			// 測試
		break;
	case 'l!002':
		flex_level.sort321_test();					// 測試
		break;
	case 'l!003':
		flex_level.getUserProfile(_event, _bot);	// 測試
		break;
	case 'l!004':
		flex_level.msg_test(_event, _bot);			// 測試
		break;	
	case 'l!005':
		flex_level.testAllColor(_event, _bot);		// 測試
		break;													
	case '?test000':
		temp = flex_level.msg();
		_event.reply(temp);
