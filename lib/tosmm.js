...
bot.on('memberJoined', function (_event) {
	...
	let gName = (Object.prototype.hasOwnProperty.call(groupDB[idx], 'gname')) ?
		(groupDB[idx].gname) : '';
	_event.joined.profiles().then(function (_profiles) {
		if(_event.source.type === 'group') {
			let replyMsg_normal = _profiles[0].displayName + s.idx_welcome;
			_event.reply(replyMsg_normal);
			fmlog('event_msg', [s.idx_mjoined, _profiles[0].displayName + s.idx_join +
				gName + s.idx_group, _event.joined.members[0].userId]);
		}
	});
});
...
bot.on('memberLeft', function (_event) {
	_event.left.profiles().then(function (_profiles) {
		let userDisplayName, userId;
		if (_event.source.type === 'group') {
			if (Array.isArray(_profiles)) {
				userDisplayName = _profiles[0].displayName;
				userId = _profiles[0].userId;
				// 沒有加機器人的User無法靠uID取得Profile資訊，只會取得 Not found訊息。
				if (_profiles[0].message === 'Not found') {
					userDisplayName = '非土司好友';
					userId = _event.left.members[0].userId;
				}
				try {
					let gID = _event.source.groupId;
					let idx = usr_mgr.getGIDX(gID);
					...
					let gName = (Object.prototype.hasOwnProperty.call(groupDB[idx], 'gname')) ?
						(groupDB[idx].gname) : '';
					fmlog('event_msg', [s.idx_mleft, userDisplayName + s.idx_leave +
						gName + s.idx_group, userId]);
				} catch (e) { console.log(e); }
			} else {
				fmlog('error_msg', [s.tosmm_memberleft, s.tosmm_profile_err, '']);
			}
		}
	});
});
