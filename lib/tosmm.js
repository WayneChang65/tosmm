...
scheduling.setCyclicTimer_Do_Everyday(undefined, bot, _saveDBs, 
	'00:05', c.TOMORROW, 'SYS DB Save!', '統計存檔 Timer ON!');

// 空氣資訊
//scheduling.setCyclicTimer_Do_Everyday(undefined, bot, air_info.getAirInfo, 
//	'07:10', c.TOMORROW, 'Wayne Msg!', '韋恩空氣 Timer ON!');

// 空氣資訊
scheduling.setCyclicTimer_Do_Everyday(undefined, bot, weather_info.updateWeatherInfoByP2R, 
	'06:10', c.TOMORROW, 'Weather!', '今日天氣 Timer ON!');

// Google Spreadsheet客製化指令更新 (自動 小妹請私)
scheduling.setCyclicTimer_Do_Everyday(undefined, bot, custom_cmd.reload, 
	'00:10', c.TOMORROW, 'Custom CMD!', '客製指令 Timer ON!');
...
bot.on('message', async function(_event) {
	...
	let ret_setGrouMgr, ret_maintainUDB, ret_maintainGDB, ret_bot_txt_handler;
	...
	case 'image':
		bot_img_handler.go(_event, bot);
		break;

	case 'video':
		bot_video_handler.go(_event, bot);
		break;

	case 'audio':
		bot_audio_handler.go(_event, bot);
		break;
	
	case 'location':
		bot_location_handler.go(_event, bot);
		break;

	case 'sticker':
		bot_sticker_handler.go(_event, bot);
		break;
